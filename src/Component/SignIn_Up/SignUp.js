...
export default function SignUp() {
  //console.log(UserService.getUsername().split(" ")[0].trim());
  const { data } = State_City_Data;
  const stateList = Object.keys(data);
  const [cityList, setCityList] = useState([]);
  const [errorMsg, setErrorMsg] = useState();
  const [Detail, setDetail] = useState({
    firstName: UserService.getUsername()?.split(" ")[0].trim() || "",
    lastName: UserService.getUsername()?.split(" ")[1].trim() || "",
    dateOfBirth: new Date(UserService.getUserDob() || "2000-01-01"),
    bloodGroup: "",
    whatsAppNumber: UserService.getUserMobile() || "",
    ...
    if (!validator.isEmail(Detail.username)) {
      ...
      const user_details = {
        appUserDTO: {
          ...
          phoneNumber: Detail.whatsAppNumber,
        },
        donorDTO: {
      ...
      RegistrationService.registerDonor(user_details)
        .then((response) => {
          console.log(response);
          setSpinner(false);
          Swal.fire({
            imageUrl: !AuthenticationService.isUserLoggedIn()
              ? `${process.env.PUBLIC_URL}/assets/email.png`
              : `${process.env.PUBLIC_URL}/assets/WYW2.png`,
            imageHeight: "200",
            imageWidth: "250",
            title: AuthenticationService.isUserLoggedIn()
            ...
            html:
              ...
              "<i>" +
                Detail.username +
                "</i> <br> Head over to your inbox to confirm your email and finish your account creation!",
              ...
              >
                <div className="form_grid">
                  ...
                  <div style={{ gridArea: "DOB" }}>
                    <LocalizationProvider dateAdapter={AdapterDateFns}>
                      <DatePicker
                        ...
                        renderInput={(params) => (
                          <TextField {...params} color="error" required />
                        )}
                      />
                  ...
                  <TextField
                    ...
                    InputProps={{
                      readOnly: AuthenticationService.isUserLoggedIn(),
                    }}
                  />
