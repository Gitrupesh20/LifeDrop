...
function VerifyAccount() {
  ...
  useEffect(() => {
    const fetch = async (token) => {
      ...
      try {
        ...
        if (response.data === "confirmed" || response.ok) setConfirm(true);
  ...
  const template = (isConfirm) => {
    ...
    var text =
      "There might be some issue with your account please register again.";
    var url = "donors/signup";

    if (isConfirm) {
      icon = "success";
      title = "Good job!";
      text = "Your have successfully registered as LifeDrop Member.";
      url = "/signin";
    }
    Swal.fire({
      icon: icon,
